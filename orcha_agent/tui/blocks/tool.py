...
def _preview(content: str, expanded: bool) -> tuple[str, bool]:
    lines = content.splitlines() or ([""] if content else [])
    clipped_columns = any(len(line) > _PREVIEW_COLUMNS for line in lines)
    if not expanded:
        lines = [line[:_PREVIEW_COLUMNS] + ("…" if len(line) > _PREVIEW_COLUMNS else "") for line in lines]
        clipped_lines = len(lines) > _PREVIEW_LINES
        lines = lines[:_PREVIEW_LINES]
        return "\n".join(lines), clipped_columns or clipped_lines
...
def render(
    ...
    title = f"{name_label}{f' · {detail}' if detail else ''}"
    elapsed = float(block.data.get("elapsed", 0.0))
    glyph = "✘" if error else ("✻" if pending else "✔")
    ...
    if isinstance(calls, list) and calls:
        ...
        if diff is not None:
            ...
            body = render_diff(diff_block, theme, 0, budget_rows, expanded)
            has_output = True
        else:
            content = _result_text(result) if result is not None else json.dumps(args, indent=2, ensure_ascii=False, default=str)
            if name == "execute" and result is not None and (code := _exit_code(result)) is not None:
                content = f"{content}\n\nExit code: {code}" if content else f"Exit code: {code}"
            content, _ = _preview(content, expanded)
            has_output = result is not None
            body = Text(content, style=str(theme_value(theme, "toolOutput")))
    if not expanded and has_output:
        body = Group(body, Text("[Ctrl+O] expand", style="dim"))
